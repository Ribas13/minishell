...
/*   Updated: 2024/01/06 18:25:50 by diosanto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_files(t_statement *node)
{
	...
	while (temp->next != NULL)
	{
		...
		if (node->has_error == false && (temp->operator == NONE || temp->operator == RDR_INPUT) && access(temp->next->argv[0], F_OK) != 0)
		{
		...
		if ((temp->operator == RDR_OUT_APPEND || temp->operator == RDR_OUT_REPLACE)
			...
			&& access(node->next->argv[0], F_OK) == 0 && access(node->next->argv[0], W_OK) == -1)
		{
			error_msg_prefix = ft_strjoin("minishell: ", temp->next->argv[0]);
			perror(error_msg_prefix);
